"use client";
import type {EthereumProvider, NexusSDK, OnAllowanceHookData, OnIntentHookData} from "@avail-project/nexus-core";
...
const useInitNexus = (sdk: NexusSDK) => {
    ...
    const initializeNexus = async () => {
        try {
            ...
            // Provide richer logging so the developer can see network / fetch failures
            if (error instanceof Error) {
                console.error("Error initializing Nexus:", error.message);
                console.error(error.stack);
            } else {
                console.error("Error initializing Nexus:", error);
            }
            // Rethrow so callers (UI) can react to the failure instead of silently swallowing it
            throw error;
        }
    };
